...
export const postTriggerDrepInfoSync = async (
  ...
  try {
    ...
    if (!acquired) {
      return res.status(202).json({ success: true, accepted: false, message: "DRep info sync is already running. Skipping duplicate trigger." });
    }

    console.log("[DRep Info Sync] Triggered via API endpoint");
    res.status(202).json({ success: true, accepted: true, message: "DRep info sync started", jobName: JOB_NAME });
